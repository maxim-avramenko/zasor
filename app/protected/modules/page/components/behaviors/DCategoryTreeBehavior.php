<?php
namespace page\components\behaviors;
...
class DCategoryTreeBehavior extends DCategoryBehavior
{
    ...
    {
        if (true === $lastLink) {
            $breadcrumbs = [$this->getOwner()->{$this->titleAttribute} => Yii::app()->createUrl('/page/page/view', ['slug' => $this->path])];
        } else {
        ...
        while ($i-- && $this->cached($page)->{$this->parentRelation}) {
            $breadcrumbs[$page->{$this->parentRelation}->{$this->titleAttribute}] = Yii::app()->createUrl('/page/page/view', ['slug' => $page->{$this->parentRelation}->path]);
